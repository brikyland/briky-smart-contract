...
describe('7.1. PrestigePad', async () => {
    ...
    async function beforePrestigePadTest({
        ...
        if (addSampleLaunch) {
            await callTransaction(getPrestigePadTxByInput_InitiateLaunch(
                prestigePad,
                manager,
                {
                    initiator: initiator1.address,
        ...
        if (addSampleRounds) {
            ...
            if (useFailReceiverAsInitiator) {
                await callTransaction(getPrestigePadTxByInput_CallUpdateRounds(prestigePad, failReceiver, params1, validator));
            } else {
                await callTransaction(getPrestigePadTxByInput_UpdateRounds(prestigePad, initiator1, params1, validator));
            }
        ...
        if (confirmFirstRound || doAllFirstFound) {
            await callTransaction(getPrestigePadTxByParams_SafeConfirmCurrentRound(
                prestigePad,
                initiator1,
                { launchId: BigNumber.from(1) },
                { value: ethers.utils.parseEther('10') }
        ...
        if (cancelFirstRound) {
            await callTransaction(getPrestigePadTx_CancelCurrentRound(prestigePad, initiator1, { launchId: BigNumber.from(1) }));
            await callTransaction(getPrestigePadTx_CancelCurrentRound(prestigePad, initiator2, { launchId: BigNumber.from(2) }));
        }
        ...
        if (finalizeLaunch) {
            await callTransaction(getPrestigePadTxByParams_SafeFinalize(
                prestigePad,
                initiator1,
                { launchId: BigNumber.from(1) },
            ));
    ...
    describe('7.1.2. updateBaseUnitPriceRange(uint256,uint256,bytes[])', async () => {
        it('7.1.2.1. Update base unit price range successfully with valid signatures', async () => {
            ...
            const tx = await getPrestigePadTxByInput_UpdateBaseUnitPriceRange(prestigePad, deployer, paramsInput, admin, admins);
            await tx.wait();
    ...
    describe('7.1.7. onERC1155Received(address,address,uint256,uint256,bytes)', async () => {
        it('7.1.7.1. Successfully receive ERC1155 tokens when receiving project token', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
        ...
        it('7.1.7.2. Revert when receiving ERC1155 tokens when receiving unknown ERC1155 token', async () => {
            const fixture = await beforePrestigePadTest();
    ...
    describe('7.1.8. onERC1155BatchReceived(address,address,uint256[],uint256[],bytes)', async () => {
        it('7.1.8.1. Successfully receive ERC1155 tokens batch when receiving project token', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
                doAllFirstFound: true,
    ...
    describe('7.1.9. initiateLaunch(address,bytes32,string,string,uint256,uint256,(uint256,uint256,bytes))', async () => {
        ...
        it('7.1.9.2. Initiate launch unsuccessfully when the contract is reentered', async () => {
            ...
            await testReentrancy_prestigePad(
                ...
                async (timestamp: number) => {
                    await expect(getPrestigePadTxByInput_InitiateLaunch(
                        prestigePad,
                        manager,
                        {
                            initiator: reentrancy.address,
        ...
        it('7.1.9.3. Initiate launch unsuccessfully by non-executive account', async () => {
            ...
            await expect(getPrestigePadTxByInput_InitiateLaunch(prestigePad, user, defaultParamsInput, validator))
        ...
        it('7.1.9.4. Initiate launch unsuccessfully when paused', async () => {
            ...
            await expect(getPrestigePadTxByInput_InitiateLaunch(prestigePad, manager, defaultParamsInput, validator))
        ...
        it('7.1.9.5. Initiate launch unsuccessfully with invalid validation', async () => {
            ...
            await expect(getPrestigePadTx_InitiateLaunch(prestigePad, manager, params))
        ...
        it('7.1.9.10. Initiate launch unsuccessfully when initiator cannot receive erc1155', async () => {
            const fixture = await beforePrestigePadTest();
    ...
    describe('7.1.10. updateLaunchURI(uint256,string,(uint256,uint256,bytes))', async () => {
        ...
        it('7.1.10.2. Update launch uri unsuccessfully with invalid launch id', async () => {
            ...
            await expect(getPrestigePadTxByInput_UpdateLaunchURI(
                prestigePad,
                manager,
                {
                    ...defaultParamsInput,
            ...
            await expect(getPrestigePadTxByInput_UpdateLaunchURI(prestigePad, manager, paramsInput2, validator))
                .to.be.revertedWithCustomError(prestigePad, 'InvalidLaunchId');
        });
        ...
        it('7.1.10.3. Update launch uri unsuccessfully when sender is not launch initiator', async () => {
            ...
            // By manager
            await expect(getPrestigePadTxByInput_UpdateLaunchURI(prestigePad, manager, defaultParamsInput, validator))
                .to.be.revertedWithCustomError(prestigePad, 'Unauthorized');

            // By wrong initiator
            await expect(getPrestigePadTxByInput_UpdateLaunchURI(prestigePad, initiator2, defaultParamsInput, validator))
                .to.be.revertedWithCustomError(prestigePad, 'Unauthorized');
        });
        ...
        it('7.1.10.4. Update launch uri unsuccessfully when paused', async () => {
            ...
            await expect(getPrestigePadTxByInput_UpdateLaunchURI(prestigePad, initiator1, defaultParamsInput, validator))
                .to.be.revertedWith('Pausable: paused');
        });
        ...
        it('7.1.10.6. Update launch uri unsuccessfully when launch is finalized', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
                doAllFirstFound: true,
    ...
    describe('7.1.11. updateRound(uint256,uint256,(string,(uint256,uint256,uint256),(uint256,address), (uint256,uint256,bytes)))', async () => {
        ...
        it('7.1.11.2. Update round unsuccessfully with invalid launch id', async () => {
            ...
            await expect(getPrestigePadTxByInput_UpdateRound(
                prestigePad,
                initiator1,
        ...
        it('7.1.11.5. Update round unsuccessfully with invalid round validation', async () => {
            ...
            await expect(getPrestigePadTx_UpdateRound(prestigePad, initiator1, params))
                .to.be.revertedWithCustomError(prestigePad, 'InvalidSignature');
        });

        it('7.1.11.6. Update round unsuccessfully when launch is finalized', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
                doAllFirstFound: true,
            ...
            await expect(getPrestigePadTxByInput_UpdateRound(prestigePad, initiator1, defaultParamsInput, validator))
                .to.be.revertedWithCustomError(prestigePad, 'AlreadyFinalized');
        });
        ...
        it('7.1.11.8. Update round unsuccessfully when updated round is already initiated', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
            });
        ...
        it('7.1.11.9. Update round unsuccessfully when currency price is not in range', async () => {
            ...
            await expect(getPrestigePadTxByInput_UpdateRound(prestigePad, initiator1, paramsInput1, validator))
            ...
            await expect(getPrestigePadTxByInput_UpdateRound(prestigePad, initiator1, paramsInput2, validator))
                .to.be.revertedWithCustomError(prestigePad, 'InvalidUnitPrice');
        });
    ...
    describe('7.1.12. updateRounds(uint256,uint256,(string,(uint256,uint256,uint256),(uint256,address),(uint256,uint256,bytes))[])', async () => {
        ...
        it('7.1.12.3. Update round unsuccessfully when sender is not launch initiator', async () => {
            ...
            // By manager
            await expect(getPrestigePadTxByInput_UpdateRounds(prestigePad, manager, defaultParamsInput, validator))
                .to.be.revertedWithCustomError(prestigePad, 'Unauthorized');

            // By wrong initiator
            await expect(getPrestigePadTxByInput_UpdateRounds(prestigePad, initiator2, defaultParamsInput, validator))
                .to.be.revertedWithCustomError(prestigePad, 'Unauthorized');
        });
        ...
        it('7.1.12.4. Update round unsuccessfully when paused', async () => {
            ...
            await expect(getPrestigePadTxByInput_UpdateRounds(prestigePad, initiator1, defaultParamsInput, validator))
                .to.be.revertedWith('Pausable: paused');
        });

        it('7.1.12.5. Update round unsuccessfully when launch is finalized', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
                doAllFirstFound: true,
        ...
        it('7.1.12.9. Update round unsuccessfully when currency price is not in range', async () => {
            ...
            await expect(getPrestigePadTxByInput_UpdateRounds(prestigePad, initiator1, paramsInput1, validator))
                .to.be.revertedWithCustomError(prestigePad, 'InvalidUnitPrice');
            ...
            await expect(getPrestigePadTxByInput_UpdateRounds(prestigePad, initiator1, paramsInput2, validator))
                .to.be.revertedWithCustomError(prestigePad, 'InvalidUnitPrice');
        });
    ...
    describe('7.1.13. scheduleNextRound(uint256,uint256,uint256,address[],uint256[],uint40,uint40)', async () => {
        ...
        it('7.1.13.3. Raise next round unsuccessfully when the contract is reentered', async () => {
            ...
            await testReentrancy_prestigePad(
                ...
                reentrancy,
                async (timestamp: number) => {
                    await expect(getPrestigePadTx_ScheduleNextRound(
                        prestigePad,
                        initiator2,
        ...
        it('7.1.13.5. Raise next round unsuccessfully when sender is not launch initiator', async () => {
            ...
            // By manager
            await expect(getPrestigePadTx_ScheduleNextRound(prestigePad, manager, defaultParams))
                .to.be.revertedWithCustomError(prestigePad, 'Unauthorized');

            // By wrong initiator
            await expect(getPrestigePadTx_ScheduleNextRound(prestigePad, initiator2, defaultParams))
                .to.be.revertedWithCustomError(prestigePad, 'Unauthorized');
        });
        ...
        it('7.1.13.6. Raise next round unsuccessfully when paused', async () => {
            ...
            await expect(getPrestigePadTx_ScheduleNextRound(prestigePad, initiator1, defaultParams))
                .to.be.revertedWith('Pausable: paused');
        });
        ...
        it('7.1.13.11. Raise next round unsuccessfully when launch is finalized', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
                doAllFirstFound: true,
    ...
    describe('7.1.14. cancelCurrentRound(uint256)', async () => {
        it('7.1.14.1. Cancel current round successfully', async () => {
            ...
            const tx1 = await getPrestigePadTx_CancelCurrentRound(prestigePad, initiator1, { launchId: BigNumber.from(launchId1) });
        ...
        it('7.1.14.2. Cancel current round unsuccessfully with invalid launch id', async () => {
            ...
            await expect(getPrestigePadTx_CancelCurrentRound(prestigePad, initiator1, { launchId: BigNumber.from(1) }))
                .to.be.revertedWithCustomError(prestigePad, 'InvalidLaunchId');
        });
        ...
        it('7.1.14.3. Cancel current round unsuccessfully when sender is not launch initiator', async () => {
            ...
            // By manager
            await expect(getPrestigePadTx_CancelCurrentRound(prestigePad, manager, { launchId: BigNumber.from(1) }))
                .to.be.revertedWithCustomError(prestigePad, 'Unauthorized');

            // By wrong initiator
            await expect(getPrestigePadTx_CancelCurrentRound(prestigePad, initiator1, { launchId: BigNumber.from(2) }))
                .to.be.revertedWithCustomError(prestigePad, 'Unauthorized');
        });
        ...
        it('7.1.14.5. Cancel current round unsuccessfully when launch is finalized', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
                doAllFirstFound: true,
    ...
    describe('7.1.15. safeConfirmCurrentRound(uint256,bytes32)', async () => {
        ...
        it('7.1.15.7. Safe confirm current round unsuccessfully when the contract is reentered', async () => {
            ...
            await testReentrancy_prestigePad(
                ...
                reentrancyERC20,
                async (timestamp: number) => {
                    await expect(getPrestigePadTxByParams_SafeConfirmCurrentRound(
                        prestigePad,
                        initiator2,
                        { launchId: BigNumber.from(launchId) },
                        { value: ethers.utils.parseEther('100') }
        ...
        it('7.1.15.12. Safe confirm current round unsuccessfully when launch is finalized', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
                doAllFirstFound: true,
        ...
        it('7.1.15.13. Safe confirm current round unsuccessfully when round is confirmed', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
                raiseFirstRound: true,
        ...
        it('7.1.15.16. Safe confirm current round unsuccessfully when sending native token to initiator failed', async () => {
            ...
            await expect(getCallTxByParams_SafeConfirmCurrentRound(
                prestigePad,
                failReceiver,
                { launchId: BigNumber.from(1) },
                { value: ethers.utils.parseEther('100') }
        ...
        it('7.1.15.17. Safe confirm current round unsuccessfully when provide fund failed', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
                raiseFirstRound: true,
    ...
    describe('7.1.16. safeFinalize(uint256,bytes32)', async () => {
        it('7.1.16.1. Finalize launch successfully', async () => {
            ...
            const tx = await getPrestigePadTxByParams_SafeFinalize(
                prestigePad,
                initiator1,
                { launchId: BigNumber.from(launchId) },
            );
        ...
        it('7.1.16.3. Finalize launch unsuccessfully when sender is not launch initiator', async () => {
            ...
            // By manager
            await expect(getPrestigePadTxByParams_SafeFinalize(
                prestigePad,
                manager,
                { launchId: BigNumber.from(1) },
            )).to.be.revertedWithCustomError(prestigePad, 'Unauthorized');
        ...
        it('7.1.16.6. Finalize launch unsuccessfully when launch is already finalized', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
                doAllFirstFound: true,
    ...
    describe('7.1.17. contributeCurrentRound(uint256,uint256)', async () => {
        ...
        it('7.1.17.2. Deposit current round successfully with erc20 currency and no cashback', async () => {
            ...
            const tx = await getPrestigePadTx_ContributeCurrentRound(
                prestigePad,
                depositor1,
                {
                    launchId: BigNumber.from(launchId),
        ...
        it('7.1.17.3. Deposit current round unsuccessfully with invalid launch id', async () => {
            ...
            await expect(getPrestigePadTx_ContributeCurrentRound(
                prestigePad,
                depositor1,
                {
                    launchId: BigNumber.from(0),
        ...
        it('7.1.17.6. Deposit current round unsuccessfully when launch is finalized', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
                doAllFirstFound: true,
    ...
    describe('7.1.18. safeContributeCurrentRound(uint256,uint256,bytes32)', async () => {
        it('7.1.18.1. Safe deposit current round successfully', async () => {
            ...
            const tx = await getPrestigePadTxByParams_SafeContributeCurrentRound(
                prestigePad,
                depositor1,
                {
                    launchId: BigNumber.from(launchId),
        ...
        it('7.1.18.2. Safe deposit current round unsuccessfully with invalid launch id', async () => {
            ...
            await expect(getPrestigePadTx_SafeContributeCurrentRound(
                prestigePad,
                depositor1,
                {
                    launchId: BigNumber.from(0),
    ...
    describe('7.1.19. withdrawContribution(uint256)', async () => {
        it('7.1.19.1. Withdraw deposit successfully', async () => {
            ...
            const tx1 = await getPrestigePadTx_WithdrawContribution(prestigePad, depositor1, { roundId: BigNumber.from(oldRoundId1) });
            const receipt1 = await tx1.wait();
            ...
            const tx2 = await getPrestigePadTx_WithdrawContribution(prestigePad, depositor2, { roundId: BigNumber.from(oldRoundId2) });
        ...
        it('7.1.19.2. Withdraw deposit unsuccessfully when the contract is reentered', async () => {
            ...
            await callTransaction(getPrestigePadTx_CancelCurrentRound(prestigePad, initiator2, { launchId: BigNumber.from(launchId) }));

            await testReentrancy_prestigePad(
                fixture,
                reentrancyERC20,
                async (timestamp: number) => {
                    await expect(getPrestigePadTx_WithdrawContribution(prestigePad, depositor1, { roundId: BigNumber.from(oldRoundId2) }))
                        .to.be.revertedWith('ReentrancyGuard: reentrant call');
                }
            );
        });
        ...
        it('7.1.19.3. Withdraw deposit unsuccessfully with invalid round id', async () => {
            ...
            await expect(getPrestigePadTx_WithdrawContribution(prestigePad, depositor1, { roundId: BigNumber.from(0) }))
                .to.be.revertedWithCustomError(prestigePad, 'InvalidRoundId');

            await expect(getPrestigePadTx_WithdrawContribution(prestigePad, depositor1, { roundId: BigNumber.from(100) }))
                .to.be.revertedWithCustomError(prestigePad, 'InvalidRoundId');
        });
        ...
        it('7.1.19.4. Withdraw deposit unsuccessfully when paused', async () => {
            ...
            await expect(getPrestigePadTx_WithdrawContribution(prestigePad, depositor1, { roundId: BigNumber.from(oldRoundId) }))
                .to.be.revertedWith('Pausable: paused'); 
        });

        it('7.1.19.5. Withdraw deposit unsuccessfully when round is confirmed', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
                raiseFirstRound: true,
            ...
            await expect(getPrestigePadTx_WithdrawContribution(prestigePad, depositor1, { roundId: BigNumber.from(roundId) }))
                .to.be.revertedWithCustomError(prestigePad, 'AlreadyConfirmed');
        });
        ...
        it('7.1.19.11. Withdraw deposit unsuccessfully when already withdrawn', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
                raiseFirstRound: true,
            ...
            await callTransaction(getPrestigePadTx_WithdrawContribution(prestigePad, depositor1, { roundId: BigNumber.from(roundId) }));
    ...
    describe('7.1.20. withdrawProjectToken(uint256,uint256)', async () => {
        it('7.1.20.1. Withdraw project token successfully with native token when qualified for cashback', async () => {
            ...
            const tx = await getPrestigePadTx_WithdrawProjectToken(
                prestigePad,
                depositor3,
        ...
        it('7.1.20.4. Withdraw zero project token when user not deposited', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
                doAllFirstFound: true,
        ...
        it('7.1.20.5. Withdraw project token unsuccessfully when the contract is reentered', async () => {
            ...
            await testReentrancy_prestigePad(
                ...
                reentrancyERC20,
                async (timestamp: number) => {
                    await expect(getPrestigePadTx_WithdrawProjectToken(
                        prestigePad,
                        depositor3,
                        { launchId: BigNumber.from(launchId), index: BigNumber.from(index) },
                    )).to.be.revertedWith('ReentrancyGuard: reentrant call');
        ...
        it('7.1.20.10. Withdraw project token unsuccessfully when user already withdrawn project token', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
                doAllFirstFound: true,
            ...
            await expect(getPrestigePadTx_WithdrawProjectToken(prestigePad, depositor1, withdrawParams))
                .to.be.revertedWithCustomError(prestigePad, 'AlreadyWithdrawn');
        });

        it('7.1.20.11. Withdraw project token unsuccessfully when withdraw fund failed', async () => {
            const fixture = await beforePrestigePadTest({
                addSampleLaunch: true,
                addSampleRounds: true,
                doAllFirstFound: true,
