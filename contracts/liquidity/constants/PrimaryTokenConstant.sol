...
library PrimaryTokenConstant {
    ...
    /// @notice Reward for a wave of staking pool 3. After culmination, reward is the minor between this value and the
    ///         remain mintable tokens to reach its cap.
    ...
    /// @notice Wave index that culminates staking pool 3, After the wave, staking costs fee but reward can still be
    ///         minted only if the total supply has not reached its cap.
